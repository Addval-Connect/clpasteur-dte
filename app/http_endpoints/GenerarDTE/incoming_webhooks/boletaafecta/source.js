...
exports = async function(payload, response) {
    ...
    let ejecutivo = await context.functions.execute("replaceDiacritics", ejecutivoURI);
  
    let ultimoFolio = await context.services.get("base-dte").db(database).collection("boleta-afecta").findOne({"ultimoFolio": true});
    ...
    boletaDTE["Encabezado"]["Receptor"]["RznSocRecep"] = await context.functions.execute("replaceDiacritics", paciente.nombre);
    ...
    if (res.status == 200) {
      ...
      await context.services.get("base-dte").db(database).collection("boleta-afecta").insertOne({
            "ultimoFolio": true,
            "folio": nuevoFolio,
            "servicios": services,
            "paciente": paciente,
      ...
      await context.services.get("base-dte").db(database).collection("boleta-afecta").updateOne({"folio": ultimoFolio.folio}, { $set: { "ultimoFolio": false }});
